...
st.title("Face Addition Section App")
st.write("In this section you will add new face with an id for tracking")

menu = ["Adding","Deleting", "Adjusting"]
choice = st.sidebar.selectbox("Options",menu)
if choice == "Adding":
    name = st.text_input("Name",placeholder='Enter name of the person')
    id = st.text_input("ID",placeholder='Enter unique id for that person')
    ...
    upload = st.radio("Choose the option to upload the picture",("Upload","Webcam"))
    if upload == "Upload":
